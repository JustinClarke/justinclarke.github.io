 ...
 *
 * Fits in: components/layout/ the always-on chrome every page renders last.
...
import { closerMetadata, SITE } from '@/content';
...
export const TheCloser = () => {
  ...
  // Scroll into view when the terminal opens. The 150ms delay lets the terminal
  // finish appearing first, so the section's new taller height is what's scrolled to.
  useEffect(() => {
    if (isTerminalOpen) {
  ...
  return (
    <SectionContainer
      ...
      className='relative overflow-hidden flex flex-col min-h-[100svh] h-auto md:h-auto md:min-h-[640px] justify-between border-t border-fg/5 pb-28 xs:pb-32 sm:pb-24 md:pb-8'
    ...
    >
      <div className='flex flex-col flex-1'>
        ...
        <div className="flex flex-col gap-4 mb-4">
          <ScrollReveal direction="right" distance={12} className="flex items-center gap-6">
            <span className="font-mono text-micro md:text-micro tracking-[0.4em] uppercase text-fg-mid font-bold whitespace-nowrap">
              Contact
            </span>
            ...
            <div className="flex items-center gap-2" data-tooltip={TOOLTIPS.livestatus} data-tooltip-pos="below">
              ...
              <span className="font-mono text-micro md:text-micro text-brand-primary font-bold uppercase tracking-widest">Live</span>
            </div>
          </ScrollReveal>
        </div>
        ...
        <div className='flex-1 flex flex-col justify-center pt-8 sm:pt-12 md:pt-0 pb-4 md:pb-0'>
          <div className='flex flex-col lg:flex-row items-center lg:items-end justify-between gap-8 sm:gap-12 lg:gap-20'>
            <ScrollReveal delay={0.1} direction="up" className='max-w-3xl'>
              <h2 className='font-noto text-4xl md:text-[clamp(28px,5vw,60px)] font-black leading-[0.95] md:leading-[1.05] tracking-tighter text-fg text-center lg:text-left'>
                If your data isn't <br className="hidden md:block" />
                <span className="text-fg-mid"> driving decisions,</span><br />
            ...
            <div className='flex flex-col items-center lg:items-end gap-6 sm:gap-8 lg:gap-10 flex-shrink-0 w-full lg:w-auto'>
              ...
              <ScrollReveal once={false} delay={0.4}>
                ...
                >
                  ...
                  <div className='relative z-10 w-7 h-7 md:w-8 md:h-8 rounded-full bg-fg/[0.03] flex items-center justify-center border border-fg/10 group-hover:bg-brand-primary/10 group-hover:border-brand-primary/40 transition-all duration-500'>
                    <svg aria-hidden="true" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-fg-mid group-hover:text-brand-primary transition-all duration-500 group-hover:-translate-y-[2px] group-hover:translate-x-[2px]"><path d="M7 17l9.2-9.2M17 17V7H7" /></svg>
                  </div>
                </button>
              </ScrollReveal>
              ...
              <ScrollReveal once={false} delay={0.5}>
                <a
                  className='font-mono text-label md:text-sm text-fg-mid font-bold leading-relaxed tracking-[0.08em] hover:text-brand-primary transition-colors duration-400 group relative cursor-pointer'
      ...
      <ScrollReveal
        delay={0.5}
        className='flex flex-col md:flex-row items-center justify-between gap-12 md:gap-12 pt-12 md:pt-16 w-full pb-6 md:pb-0 border-t border-fg/5 mt-16 md:mt-24'
      >
        <Link
          to="/"
          onClick={(e) => {
        ...
        >
          <div className='w-10 h-10 md:w-12 md:h-12 rounded-full bg-fg/[0.03] border border-fg/[0.08] flex items-center justify-center transition-all duration-500 group-hover/logo:border-brand-primary/40 group-hover/logo:bg-brand-primary/5'>
            <span className='font-mono text-caption md:text-sm font-bold text-brand-primary/90'>{SITE.firstName[0]}{SITE.lastName[0]}</span>
          </div>
          <div className='flex flex-col'>
            <span className='font-mono text-micro md:text-fine font-bold tracking-[0.1em] uppercase text-fg-soft'>{SITE.name.toUpperCase()}</span>
            <span className='font-mono text-micro md:text-micro text-fg/15 uppercase tracking-[0.25em] font-medium'>{/* tw-allow-contrast */}© 2026_EST</span>
          </div>
        </Link>

        <nav aria-label='Social Connectivity'>
          <ul className='flex items-center gap-3 md:gap-4'>
            <li><FooterSocialLink href={`https://github.com/${SITE.social.github}`} icon={Github} label='GitHub' /></li>
            <li><FooterSocialLink href={`https://linkedin.com/in/${SITE.social.linkedin}`} icon={Linkedin} label='LinkedIn' /></li>
        ...
        <div className='flex flex-col items-center md:items-end gap-1.5'>
          <span className='font-mono text-micro md:text-micro font-bold text-fg/10 uppercase tracking-ultra'>{/* tw-allow-contrast */}Source_Manifest</span>
          <a
            href={`https://github.com/${SITE.social.github}/${SITE.domain}`}
            target='_blank'
            rel='noopener noreferrer'
            data-tooltip={TOOLTIPS.sourcemanifest}
            data-tooltip-pos="below"
            className='font-mono text-caption md:text-sm font-bold tracking-[0.05em] text-fg-mid hover:text-brand-primary transition-colors group'
          >
            <span className='smooth-underline after:h-[1px] after:bg-brand-primary'>{SITE.domain}</span>
          </a>
        </div>
      </ScrollReveal>
      ...
      <ScrollReveal
        delay={0.6}
        className='w-full border-t border-fg/5 pt-12 mt-16'
      >
        <div className='flex flex-col gap-4 font-mono text-micro md:text-micro w-full border border-fg/[0.04] bg-fg/[0.01] hover:bg-fg/[0.015] rounded-xl p-4 md:p-6 transition-all duration-300 backdrop-blur-md relative overflow-hidden group/compliance shadow-[0_4px_30px_rgba(0,0,0,0.4)]'>

          {/* Header Bar */}
          <div className='flex items-center justify-between gap-3 text-fg-mid uppercase tracking-[0.15em] font-semibold border-b border-fg/[0.04] pb-3'>
            <div className='flex items-center gap-2 xs:gap-3 min-w-0'>
              ...
              <div className='h-3 w-px bg-fg/10 hidden xs:block shrink-0' />
              <div className='flex items-center gap-1.5 xs:gap-2 min-w-0'>
                <Shield className='w-3.5 h-3.5 text-brand-primary/70 animate-pulse shrink-0' />
                <span className='text-brand-primary/70 font-bold truncate text-micro xs:text-micro sm:text-micro'>COMPLIANCE_PROTOCOL</span>
              </div>
            </div>
            <div className='h-px flex-1 bg-fg/[0.03] hidden lg:block' />
            <div className='flex items-center gap-3 text-micro md:text-micro shrink-0'>
              ...
              <span className='bg-fg/[0.03] px-2 py-0.5 rounded border border-fg/5 text-fg/20 hidden sm:inline'>{/* tw-allow-contrast */}VER: 2026_EST</span>
            </div>
          </div>

          {/* Structured Detail Grid */}
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 pt-2'>
            <div className='flex flex-col gap-2 pl-3 border-l-2 border-fg/5 hover:border-brand-primary/45 transition-colors duration-300 group/card'>
              <div className='flex items-center gap-2 text-fg-mid font-bold text-micro md:text-micro tracking-[0.08em]'>
                <span className='text-brand-primary/60 font-mono'>01_</span>
                <span>PROJECT_CLASSIFICATION</span>
              </div>
              <p className='text-fg-mid leading-relaxed font-sans text-fine md:text-caption'>
            ...
            <div className='flex flex-col gap-2 pl-3 border-l-2 border-fg/5 hover:border-brand-primary/45 transition-colors duration-300 group/card'>
              ...
              <p className='text-fg-mid leading-relaxed font-sans text-fine md:text-caption mb-1'>
              ...
              <div className='flex flex-wrap gap-1.5 pt-1'>
                <span className='font-mono text-micro md:text-micro font-bold bg-viz-mac-red/5 border border-viz-mac-red/15 text-viz-mac-red px-2 py-0.5 rounded tracking-wider uppercase'>F1_TELEMETRY</span>
                <span className='font-mono text-micro md:text-micro font-bold bg-viz-mac-yellow/5 border border-viz-mac-yellow/15 text-viz-mac-yellow px-2 py-0.5 rounded tracking-wider uppercase'>AUDI_SPEC</span>
                <span className='font-mono text-micro md:text-micro font-bold bg-viz-spotify/5 border border-viz-spotify/15 text-viz-spotify px-2 py-0.5 rounded tracking-wider uppercase'>SPOTIFY_API</span>
              </div>
            </div>
          ...
          <div className='mt-2 pt-4 border-t border-fg/[0.04] flex flex-col gap-3'>
            <div className='flex flex-col gap-3 sm:flex-row sm:items-center justify-between'>
              <span className='text-fg-faint text-micro md:text-micro tracking-mega font-semibold uppercase self-start sm:self-auto'>
              ...
              <button
                ...
                className='flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-fg/[0.02] border border-fg/[0.06] text-fg-mid hover:text-brand-primary hover:border-brand-primary/30 transition-all duration-300 hover:bg-brand-primary/[0.02] cursor-pointer text-micro md:text-micro tracking-wider uppercase self-start sm:self-auto'
              >
            ...
            {/* The audit console is an intentionally dark terminal panel, so it
                opts out of theming its fg tokens re-pin light-on-dark in light
                mode via data-theme-lock. */}
            {isTerminalOpen && (
              <ScrollReveal
                direction='up'
                distance={6}
                data-theme-lock="dark"
                className='w-full border border-brand-primary/10 bg-[#060808]/95 rounded-lg p-4 font-mono text-micro text-fg-mid leading-relaxed shadow-inner overflow-x-auto relative scanline'
              >
                <div className='flex flex-col gap-1.5 min-w-[280px]'>
                  <div className='flex items-center gap-2 text-brand-primary/80 font-bold'>
                    <span>$</span>
                    <span>compliance-check --verbose --target={SITE.domain}</span>
                  </div>
                  <div className='text-fg-mid font-semibold'>[OK] INITIALIZING COMPLIANCE & PRIVACY AUDIT...</div>
                  <div>[OK] PROJECT STATUS: NON-COMMERCIAL / PROFESSIONAL PORTFOLIO</div>
                  <div className='flex flex-col gap-0.5 pl-3'>
                    <div className='text-fg-soft'>[INFO] ASSET IDENTIFICATION SCAN:</div>
                    <div className='text-fg-mid pl-2'>
                  ...
                  <div className='flex flex-col gap-0.5 pl-3'>
                    <div className='text-fg-soft'>[OK] PRIVACY PROTOCOL VALIDATION:</div>
                  ...
                  <div className='flex items-center gap-2 text-viz-success/90 font-bold mt-1 pt-1.5 border-t border-fg/[0.04]'>
...
// Lookup tables so FooterSocialLink resolves its tooltip and brand colours by
// label instead of repeating five near-identical copies of the markup.
...
const FooterSocialLink = ({
  ...
  // Neutral grey fallback so a label with no BRAND_STYLES entry still renders.
  const styles = BRAND_STYLES[label] || {
    default: 'bg-fg/[0.02] border-fg/5 text-fg-mid',
  ...
  return (
    <a
      href={href}
      onClick={handleClick}
      // Links with a custom onClick stay in-tab; plain links open a new tab
      // (with the standard rel guard for third-party sites).
