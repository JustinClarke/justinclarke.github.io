/**
 * tooltipContent.ts the actual tooltip TEXT, keyed by a normalised label.
 *
 * Fits in: getTooltip('Power BI') -> the copy, used by Badge and the tooltip
 *          system in tooltips.ts (which handles SHOWING them) both live
 *          in utils/ now so the pair isn't split across two layers.
