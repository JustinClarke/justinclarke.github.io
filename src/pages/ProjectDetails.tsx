 ...
 * ProjectDetails Page
 * 
 * Transition from gallery to a dedicated, high-fidelity case study page.
 * 
 * Architecture:
 * - Hero: Narrative section with performance metrics and a responsive visual block.
 * - Case Study: Detailed breakdown (Problem, Approach, Outcome) in a grid.
 * - Colors: Fully migrated to semantic tokens (brand-bg, brand-card, border-studio).
 */
...
import { TheCloser } from '@/components/ui-global/TheCloser';
...
function ProjectDetailsSkeleton() {
  return (
    <div className="w-full min-h-screen bg-brand-bg flex flex-col pt-24 items-center p-6">
...
export function ProjectDetails() {
  ...
  return (
    <div className="min-h-screen bg-brand-bg text-white flex flex-col font-sans selection:bg-brand-primary/30 overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col items-center">
        ...
        {/* ── Hero Section: Narrative and impact summary ── */}
        <SectionContainer
          className="w-full border-b border-border-studio bg-brand-card"
        ...
        >
          ...
          <motion.div 
            ...
            className="p-8 md:p-12 lg:p-16 flex flex-col gap-6 md:gap-8 border-b lg:border-b-0 lg:border-r border-border-studio"
          >
            <div className="flex flex-col gap-4">
              <span className="font-mono text-[12px] md:text-[13px] font-black tracking-[0.2em] uppercase text-brand-primary/90">
                {project.projectType}
              </span>
            ...
            {/* Metrics Dashboard Row: Proof of impact at a glance */}
            {project.pageMetrics && (
              <div className="mt-8 md:mt-12 pt-8 border-t border-border-studio grid grid-cols-2 sm:flex sm:flex-wrap gap-x-12 gap-y-8 items-start">
          ...
          {/* Hero Visual Block: Interactive telemetry/dataviz */}
          <motion.div 
            initial={{ opacity: 0, scale: 0.99, y: 8 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            transition={{ duration: 1.2, delay: 0.1, ease: [0.25, 1, 0.5, 1] }}
            className="bg-brand-card flex items-center justify-center relative overflow-hidden min-h-[350px] md:min-h-[450px] p-8 md:p-16 lg:p-24"
          >
        ...
        {/* ── Case Study In-Depth: Problem, Approach, Outcome ── */}
