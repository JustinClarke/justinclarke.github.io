    ...
    <section id={id} ref={ref} className="w-full scroll-mt-32 reveal-element">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-graphite-800/80 backdrop-blur-md border border-white/10 rounded-2xl p-8 lg:p-10 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-noto font-black text-text-primary mb-4 uppercase tracking-tighter leading-none">
              Five XGBoost Models.
            </h2>
          ...
          <div className="mt-6 pt-6 border-t border-white/10 flex flex-col gap-2 font-jetbrains text-micro text-text-tertiary">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-text-tertiary">{ML_FACTS.meanIntervalWidth}s</span>
            </div>
            <div className="flex justify-between">
              <span>ONNX parity</span>
              <span className="text-text-tertiary">all 5 boosters · atol=1e-5</span>
            </div>
            <div className="flex justify-between">
      ...
      <div className="mt-6 relative overflow-hidden rounded-2xl border border-amber-500/25 bg-amber-500/[0.04] p-6 md:p-8 flex flex-col md:flex-row md:items-center gap-5">
        ...
        <div className="shrink-0 flex flex-col items-center md:items-start gap-1 relative z-10">
          <span className="font-jetbrains text-micro uppercase tracking-mega text-amber-400/80 font-semibold">Adversarial leakage probe</span>
          <div className="flex items-baseline gap-2 font-jetbrains">
            ...
            <span className="text-text-tertiary text-sm tabular-nums">vs {ML_FACTS.leakageMajority} majority</span>
          </div>
        </div>
        <p className="font-jetbrains text-fine md:text-xs leading-relaxed text-text-secondary relative z-10">
          A classifier trained to recover <code className="text-amber-400">race_year</code> from the
          feature set hits <span className="text-text-primary font-semibold">{ML_FACTS.leakageProbeAccuracy}</span> accuracy
          against a <span className="text-text-primary font-semibold">{ML_FACTS.leakageMajority}</span> majority-class
...
const ModelCard: React.FC<ModelCardProps> = ({ model }) => {
  // beatsPct's sign/direction is chosen by whether the metric is "up good" or "down good".
  ...
  return (
    <div className="flex-1 bg-graphite-800/40 border border-white/5 rounded-xl px-4 md:px-6 py-4 hover:bg-graphite-800/70 hover:border-white/10 transition-all duration-300 relative overflow-hidden group flex items-center gap-3 md:gap-5 min-h-[72px]">
      ...
      <div className="flex-1 min-w-0">
        <h3 className="font-jetbrains text-xs font-bold text-text-primary uppercase tracking-wide flex items-center gap-2 mb-1">
          <span className="w-1.5 h-1.5 rounded-full bg-f1-red shrink-0" />
          {model.label}
        </h3>
        <div className="font-jetbrains text-micro text-text-tertiary uppercase tracking-wider truncate">{model.name}</div>
      </div>
      {/* beats-baseline bar chart */}
      <div className="shrink-0 w-36 sm:w-44 md:w-52 font-jetbrains">
        <div className="flex items-center justify-between text-micro uppercase tracking-wider mb-1.5">
          <span className="text-text-tertiary">{model.metric}</span>
          <span className="text-emerald-400 font-bold">{beatsPct}</span>
        </div>
        <div className="flex items-center gap-2 mb-1">
          <span className="w-7 text-micro text-text-tertiary uppercase shrink-0">base</span>
          <div className="flex-1 h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
            <div className="h-full rounded-full bg-white/25 transition-[width] duration-500" style={{ width: `${basePct}%` }} />
          </div>
          <span className="w-9 text-micro text-text-tertiary tabular-nums text-right shrink-0">{baselineStr}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-7 text-micro text-emerald-400/80 uppercase shrink-0">eval</span>
          <div className="flex-1 h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
            <div className="h-full rounded-full bg-emerald-500 transition-[width] duration-500" style={{ width: `${evalPct}%` }} />
          </div>
          <span className="w-9 text-micro text-white font-bold tabular-nums text-right shrink-0">{evalStr}</span>
        </div>
      </div>
      {/* Built chip */}
      <span className="hidden md:inline-flex shrink-0 font-jetbrains text-micro uppercase tracking-wider border px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border-emerald-500/20 items-center">
        ✓ Built
      </span>
    </div>
  );
