...
export function CausalPanel({ decomp, fuelWeight }: CausalPanelProps) {
  return (
    // Enters from the left (x: -20 → 0), mirroring TelemetryPanel so the two
    // slide past each other on toggle.
    <motion.div
    ...
    >
      <div className="flex items-center justify-between border-b border-[rgba(255,255,255,0.05)] pb-1">
        <span className="font-mono text-micro xs:text-micro lg:text-micro xl:text-micro 2xl:text-micro text-neutral-500 uppercase tracking-wider lg:tracking-tight xl:tracking-normal 2xl:tracking-wider whitespace-nowrap shrink-0">CAUSAL DECOMP (ε)</span>
        <span className="font-mono text-micro xs:text-micro lg:text-micro xl:text-micro 2xl:text-xs text-white font-bold shrink-0">{decomp.totalLapTime.toFixed(3)}s</span>
      </div>

      <div className="flex flex-col gap-1 text-micro xs:text-micro lg:text-micro xl:text-micro 2xl:text-micro font-mono leading-tight">
        {/* Expected Stint wear */}
        ...
        {/* Air state a positive penalty (slower) shows red; a negative one
            (a gain from a tow/DRS) shows green. */}
        <div className="flex justify-between items-center gap-2 whitespace-nowrap">
