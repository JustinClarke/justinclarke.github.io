  ...
  return (
    <div className="md:col-span-3 flex flex-col justify-between bg-black/40 border border-[rgba(255,255,255,0.05)] rounded-2xl p-2.5 lg:p-3 relative overflow-hidden min-h-0">
      ...
      <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 z-10 shrink-0">
        <span className={cn(
          "font-mono text-micro md:text-micro font-black px-2 py-0.5 rounded uppercase border",
        ...
        {isCliffRisk && (
          <span className="font-mono text-micro md:text-micro font-black px-2 py-0.5 rounded bg-[rgba(245,158,11,0.1)] border border-[rgba(245,158,11,0.3)] text-amber-400 animate-pulse flex items-center gap-1">
      ...
      <div className="relative w-full flex-grow flex items-center justify-center min-h-[220px] md:min-h-[280px] mt-3">
        <svg className="w-full h-full max-h-[260px] md:max-h-[320px] p-1" viewBox={getTrackViewBox()}>
          ...
          {/* Dynamic telemetry path bright gradient stroke over the faint base
              path. Keyed on activeTrack so switching circuits replays its fade-in. */}
          <motion.path
            key={activeTrack}
            d={getTrackPath()}
            fill="none"
          ...
          {/* Racing dot (the car). Native SVG <animateMotion> loops it along the
              track path browser-driven, so React isn't involved frame-by-frame;
              `dur` is the per-circuit lap time. */}
          {isPlaying && (
            <circle
              key={`dot-${activeTrack}`}
              r="8"
      ...
      <div className="flex gap-2.5 pt-2.5 border-t border-[rgba(255,255,255,0.05)] shrink-0" onClick={(e) => e.stopPropagation()}>
        {/* Track select. stopPropagation keeps a tap from also clicking the whole card. */}
        <div className="flex gap-1.5 w-full">
          {(['Monaco', 'Spa', 'Monza'] as const).map(track => (
            <button
              ...
              className={cn(
                "flex-1 font-mono text-micro md:text-xs py-1 rounded border tracking-wider transition-all",
                activeTrack === track
