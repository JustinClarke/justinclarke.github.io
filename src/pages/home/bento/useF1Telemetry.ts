...
import { INITIAL_PIPELINE_LOGS } from './f1';
import { debug } from '@/utils';

// Silent-by-default logger (localStorage.debug = 'f1'); see utils/debug.ts.
const log = debug('f1');
...
export function useF1Telemetry() {
  ...
  // Bails when paused or running tests (no timer created), else a 400ms interval.
  useEffect(() => {
    if (!isPlaying || isTesting) return;
    log('physics loop running', { activeTrack });

    const interval = setInterval(() => {
      // Sine wave wobbles the speed smoothly over time instead of randomly.
      const time = Date.now();
      ...
      // Fuel burn simulation (~0.03kg per step, floored at 5kg)
  ...
  useEffect(() => {
    ...
    const lapInterval = setInterval(() => {
      setCurrentLap(prev => {
        // Rolling back to lap 1 means a fresh set of tyres (age 0); any other
        // lap ages the current set by 1.
  ...
  // A one-shot animation built from staggered setTimeouts (not a loop), so the
  // console looks like it's working through the tests live.
  const runDbtTests = () => {
    if (isTesting) return;
    log('dbt tests: start');
    setIsTesting(true);
    ...
    tests.forEach((t, index) => {
      // Each line lands at (index+1)*1200ms test 1 at 1.2s, test 2 at 2.4s, …
      setTimeout(() => {
  ...
  // Each track is a pre-drawn SVG path string; the widget strokes it and animates
  // the car dot along it. Just data nothing to read in the coordinates.
  const getTrackPath = () => {
    switch (activeTrack) {
  ...
  // The causal model the showpiece of this card. Breaks a lap time into named
  // causes (fuel, tyres, dirty air, tyre temp) added to a base pace, so the UI
  // can show WHY the lap was the time it was. The parts sum to totalLapTime.
  ...
  // Derived every render. The cliff values flag when soft/medium/hard tyres have
  // aged past the point where grip falls off a "cliff".
  ...
  // One big object of state, setters, and derived results; consumers Pick their slice.
  return {
    // playback + track
...
/** The full object returned by {@link useF1Telemetry}. Child panels Pick their
 *  slice, so their prop types stay in sync with this hook automatically. */
