...
import type { RoutePath } from '@/content';

import { F1TelemetryWidget } from './F1TelemetryWidget';
import { LiteStoreWidget } from './LiteStoreWidget';
import { SpotifyWidget } from './SpotifyWidget';
import { SqlDisasterWidget } from './SqlDisasterWidget';
import { BehaviouralRiskWidget } from './BehaviouralRiskWidget';
...
interface BentoCardProps {
  ...
  borderColor: string;
  gradientFrom: string;
  /** Typed against RoutePath so deleting a case-study route is a compile error here too. */
  to: RoutePath;
}
...
function BentoCard({
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-mono text-micro font-black uppercase tracking-wider">View Project</span>
...
export function FeaturedProjects() {
  ...
  // Interval/rAF ids live in refs so cleanup can cancel them without re-rendering.
  const rafRef = useRef<number | null>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const cycleRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Effect 1 auto-cycle the active card every 4 seconds.
  useEffect(() => {
  ...
  // Effect 2 the cursor spotlight. mousemove writes the cursor position into
  // CSS vars on <html> so each card's gradient tracks it without React
  // re-rendering; the rAF guard caps work at one event per repaint, and the idle
  // timer recentres the spotlight after 2s of stillness.
  useEffect(() => {
  ...
  // Factory collecting each BentoCard's props (shadow/border/gradient + computed
  // isActive) into one object, so the grid markup below stays readable.
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-4 relative z-10">
                  ...
                  <div className="flex flex-col overflow-hidden h-5 relative">
                    <div className="transition-transform duration-500 cubic-bezier(0.16, 1, 0.3, 1) group-hover:-translate-y-5">
                      ...
                      <span className="font-mono text-micro xs:text-micro sm:text-xs md:text-sm lg:text-micro xl:text-fine 2xl:text-xs font-black tracking-[0.05em] xs:tracking-[0.12em] sm:tracking-mega md:tracking-[0.25em] lg:tracking-[0.1em] xl:tracking-[0.15em] 2xl:tracking-mega text-fg-soft group-hover:text-fg uppercase transition-colors whitespace-nowrap block h-5 leading-5">
                        LET'S WORK TOGETHER
                      </span>
                      {/* State 2: Direct Recruiter Callout */}
                      <span className="font-mono text-micro xs:text-micro sm:text-xs md:text-sm lg:text-micro xl:text-fine 2xl:text-xs font-black tracking-[0.05em] xs:tracking-[0.12em] sm:tracking-mega md:tracking-[0.25em] lg:tracking-[0.1em] xl:tracking-[0.15em] 2xl:tracking-mega text-brand-primary uppercase whitespace-nowrap block h-5 leading-5">
                ...
                <div className="flex items-center gap-3 relative z-10 shrink-0">
                  {/* Availability Badge */}
                  <span className="hidden sm:inline-flex items-center px-2.5 py-0.5 rounded-md border border-acc-lang/20 bg-acc-lang/5 font-mono text-micro text-acc-lang font-black tracking-wider select-none cursor-help" data-tooltip="For the UAE">
