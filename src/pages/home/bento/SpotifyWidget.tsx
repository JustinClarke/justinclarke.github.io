  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        <span className="font-mono text-micro md:text-micro uppercase font-black tracking-wider text-fg-faint">{label}</span>
        <span className={cn(
          "font-mono text-micro md:text-micro font-black tabular-nums",
    ...
    <div className="flex flex-col h-full justify-between gap-1.5 md:gap-2 min-h-0 text-left select-none">
      ...
      <div className="flex flex-col gap-1 w-full border-b border-edge-soft pb-2 shrink-0">
        <div className="flex flex-wrap items-center gap-2 pt-0.5">
          ...
          <span className="font-mono text-micro sm:text-micro font-bold border border-viz-spotify/40 text-viz-spotify px-1.5 py-0.5 rounded uppercase tracking-wider shrink-0 bg-viz-spotify/5">
            MSC RESEARCH
          </span>
      ...
      <div className="hidden lg:flex flex-col gap-2 grow min-h-0">
        ...
        <div className="bg-bento-inset border border-edge-soft rounded-xl p-2.5 md:p-3 flex flex-col gap-2 shrink-0">
          <span className="font-mono text-micro text-fg-faint uppercase tracking-widest">Acoustic Feature Vector</span>
        ...
        <div className="bg-bento-inset border border-edge-soft rounded-lg p-2.5 md:p-3 flex flex-col gap-1.5 grow justify-center">
          <span className="font-mono text-micro text-fg-faint uppercase tracking-widest">3-Model Benchmark</span>
          <div className="flex items-center gap-1.5 flex-wrap">
            {MODELS.map((m) => (
              <span
                key={m.label}
                className={cn(
                  "font-mono text-micro md:text-micro font-black uppercase px-2 py-1 rounded-full border",
                  m.active
      ...
      <div className="lg:hidden flex flex-wrap gap-1 mt-1 shrink-0">
        {['Python', 'Scikit-Learn', 'Vector'].map(tech => (
          <span key={tech} className="font-mono text-micro sm:text-micro px-2 py-0.5 bg-viz-spotify/10 border border-viz-spotify/30 rounded text-viz-spotify font-bold uppercase tracking-wider">
            {tech}
          </span>
        ))}
      </div>
