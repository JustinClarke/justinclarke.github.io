...
const STACK = [
  ...
  { l: 'Repository',  v: 'GitHub ↗', href: 'https://github.com/JustinClarke/yourHRarchetype' },
];
...
const Label = ({ children, dark = false }: { children: React.ReactNode; dark?: boolean }) => (
  <span className={cn('inline-flex items-center gap-2 font-mono text-micro md:text-micro tracking-[0.45em] uppercase font-black', dark ? 'text-text-ghost' : 'text-black/25')}>
 ...
 * Axes are evenly spaced around a circle (angle = i/total * 2π); the data
 * value scales the radius outward, so a 0.91 score reaches 91% of max radius.
 */
      ...
      {ARCHETYPE_WEIGHTS.map((arch, i) => {
        ...
        return (
          <div key={arch.name} className="flex items-center gap-3">
            <span className="font-mono text-micro text-text-tertiary w-[120px] shrink-0 uppercase tracking-wider truncate">{arch.name}</span>
            ...
            <span className="font-mono text-micro text-text-tertiary tabular-nums w-6 text-right">{arch.w}%</span>
          </div>
        );
      })}
    </div>
...
export const HRArchetypePage = () => {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-surface text-fg font-sans selection:bg-black/10 overflow-x-hidden">
      <SEO {...routeMeta('/project/hr-archetype')} />
      <GrainOverlay />
      ...
      <div className="fixed top-10 right-10 z-[100] hidden md:flex gap-3 items-center" role="status" aria-live="polite">
        <div className="px-4 py-2 rounded-full border border-white/10 bg-black/40 backdrop-blur-xl flex items-center gap-3 shadow-2xl">
          ...
          <span className="font-mono text-micro tracking-[0.35em] uppercase text-text-tertiary font-bold">Sync · 40ms · Live</span>
        </div>
        <a
          href="https://github.com/JustinClarke/yourHRarchetype"
          target="_blank" rel="noopener noreferrer"
          className="px-4 py-2 rounded-full border border-white/10 bg-black/40 backdrop-blur-xl text-text-tertiary hover:text-white font-mono text-micro font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all duration-500 shadow-2xl flex items-center gap-1.5"
        >
        ...
        <a
          ...
          target="_blank" rel="noopener noreferrer"
          className="px-5 py-2 rounded-full bg-white text-black font-mono text-micro font-black uppercase tracking-widest hover:bg-black hover:text-white transition-all duration-500 shadow-2xl border border-black/5"
        >
          Take Diagnostic ↗
        </a>
      </div>
            ...
            <div className="flex flex-col items-center text-center relative z-10">
              ...
              >
                <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full border border-white/[0.08] bg-white/[0.03] backdrop-blur-2xl">
                  <span className="font-mono text-micro md:text-micro tracking-[0.55em] uppercase font-black text-text-tertiary">
              ...
              <motion.p
                ...
                className="font-mono text-base md:text-xl text-text-tertiary max-w-2xl leading-relaxed font-medium mb-12 md:mb-16 px-4 md:px-0"
              >
              ...
              >
                <MagneticButton as="div">
                  <a
                    ...
                    target="_blank" rel="noopener noreferrer"
                    className="px-8 md:px-10 py-4 rounded-2xl bg-white text-black font-mono text-fine font-black tracking-widest uppercase shadow-[0_0_40px_rgba(255,255,255,0.12)] hover:shadow-[0_0_56px_rgba(255,255,255,0.2)] transition-shadow block"
                  >
                    Launch Dashboard
                  </a>
                </MagneticButton>
                <MagneticButton as="div">
                  <a
                    href="https://yourarchetype.vercel.app/"
                    target="_blank" rel="noopener noreferrer"
                    className="px-8 md:px-10 py-4 rounded-2xl border border-white/15 bg-white/4 text-white font-mono text-fine font-black tracking-widest uppercase backdrop-blur-xl hover:bg-white/[0.07] transition-colors block"
                  >
                    Take the Diagnostic
                  </a>
                </MagneticButton>
                <MagneticButton as="div">
                  <a
                    href="https://github.com/JustinClarke/yourHRarchetype"
                    target="_blank" rel="noopener noreferrer"
                    className="px-8 md:px-10 py-4 rounded-2xl border border-white/15 bg-white/4 text-white font-mono text-fine font-black tracking-widest uppercase backdrop-blur-xl hover:bg-white/[0.07] transition-colors block flex items-center gap-2 justify-center"
                  >
          ...
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center border-t border-white/[0.07] pt-10 md:pt-12 gap-10">
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-3">
                ...
                <span className="font-mono text-micro tracking-[0.35em] uppercase font-black text-text-tertiary">System_Runtime_Logs</span>
              </div>
              <div className="font-mono text-fine text-text-tertiary h-5 flex items-center">
            ...
            <div className="flex gap-10 md:gap-16">
              {QUANT_SCORES.slice(0, 3).map(stat => (
                <div key={stat.label} className="flex flex-col gap-1 text-right">
                  <span className="font-mono text-micro text-text-tertiary uppercase font-black tracking-widest">{stat.label}</span>
                ...
                <div className="flex flex-wrap gap-4 pt-2">
                  {(Object.entries(TIER_META) as [RiskTier, typeof TIER_META[RiskTier]][]).map(([, meta]) => (
                    <div key={meta.label} className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-micro uppercase font-black text-black/30 tracking-widest">{meta.label}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <ScrollReveal key={arch.name} delay={i * 0.04}>
                  ...
                  >
                    <div className="flex items-center justify-between mb-6">
                      <span className="font-mono text-micro uppercase font-black text-black/20 tracking-widest">{String(i + 1).padStart(2, '0')}</span>
                      <span className={cn(
                        'font-mono text-micro uppercase font-black tracking-widest px-2.5 py-1 rounded-full border transition-opacity',
                        meta.color, meta.border, hoveredArch === i ? 'opacity-100' : 'opacity-50'
                      )}>
                        {meta.label}
                      </span>
                    </div>
                    <span className="font-mono text-micro uppercase font-black text-black/25 mb-3 leading-relaxed">{arch.tag}</span>
                    <h3 className="font-noto text-xl md:text-2xl font-black uppercase mb-3 leading-none flex-1">{arch.name}</h3>
                    <p className="font-mono text-fine text-black/35 leading-relaxed">{arch.desc}</p>
                    <motion.div
            ...
            <div className="flex flex-col gap-10 md:gap-12">
              <div className="flex flex-col gap-5">
                ...
                <h2 className="font-noto text-5xl md:text-[6.5rem] font-black tracking-tighter uppercase leading-[0.82]">
                  Engine<br />
                  <span className="text-text-ghost">Scoring.</span>
                </h2>
              </div>
                ...
                {QUANT_SCORES.map((score, i) => (
                  <div key={score.label} className="flex flex-col">
                    <div className="flex items-baseline justify-between mb-1">
                      <span className="font-mono text-micro uppercase font-black text-text-tertiary tracking-widest">{score.label}</span>
                      <span className="font-noto text-lg font-black tabular-nums">{score.value}</span>
                    </div>
                    <ScoreBar pct={score.pct} delay={i * 0.08} />
                    <span className="font-mono text-micro text-text-tertiary mt-1.5 leading-tight">{score.desc}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <ScrollReveal direction="right">
            <div className="flex flex-col gap-10">
              ...
              <div className="relative p-8 rounded-[28px] bg-white/[0.03] border border-white/[0.08] overflow-hidden">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <span className="font-mono text-micro uppercase font-black text-text-tertiary tracking-widest block mb-1">13-Axis · Diagnostic</span>
                    <h4 className="font-noto text-lg font-black uppercase">Behavioural Radar</h4>
                  </div>
                  <span className="font-mono text-micro uppercase font-black tracking-widest px-3 py-1.5 rounded-full bg-white/[0.06] text-text-tertiary">Purposeful Driver</span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-0 border-t border-white/[0.06] mt-4 pt-5">
                  {[{ v: '90 Days', l: 'Window' }, { v: '12.4%', l: 'Attrition Rate' }, { v: '94.2%', l: 'Accuracy' }].map(m => (
                    <div key={m.l} className="flex flex-col items-center text-center px-2">
                      ...
                      <span className="font-mono text-micro uppercase text-text-tertiary tracking-widest mt-0.5">{m.l}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-8 rounded-[28px] bg-white/[0.03] border border-white/[0.08]">
                <div className="mb-5">
                  <span className="font-mono text-micro uppercase font-black text-text-tertiary tracking-widest block mb-1">Realistic Workplace</span>
              ...
              {RISK_MATRIX.map((q, i) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className={cn('font-mono text-micro uppercase font-black tracking-widest', q.color)}>{q.quad}</span>
                    <div className="flex gap-2 font-mono text-micro uppercase text-black/25">
                      <span>Eng: {q.eng}</span>
                      <span>·</span>
                      <span>Risk: {q.risk}</span>
                    </div>
                  </div>
                  <p className="font-mono text-fine text-black/40 leading-relaxed">{q.action}</p>
                  <motion.div
          ...
          <ScrollReveal direction="up">
            <div className="relative p-8 md:p-12 rounded-[28px] md:rounded-[40px] bg-pitch text-white overflow-hidden shadow-[0_24px_80px_rgba(0,0,0,0.25)] group">
              ...
              <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                <div className="flex flex-col gap-5">
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-micro uppercase font-black tracking-widest px-3 py-1.5 rounded-full bg-white text-black">Gemini AI · Live Insight</span>
                  </div>
                  <p className="font-mono text-sm font-medium italic leading-relaxed text-text-secondary">
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex justify-between items-center font-mono text-micro font-black uppercase">
                    <span className="text-text-tertiary">Intervention Lead Time</span>
                    <span>90 Days</span>
                  </div>
                  ...
                  <p className="font-mono text-micro text-text-tertiary uppercase font-bold">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-3 border-t border-white/[0.06]">
                    {[{ v: '7-panel', l: 'Dashboard' }, { v: 'onSnapshot', l: 'Realtime' }, { v: 'Arrow-Key', l: 'Presentation Mode' }, { v: 'CSV + TXT', l: 'Export' }].map(m => (
                      <div key={m.l} className="flex flex-col">
                        ...
                        <span className="font-mono text-micro uppercase text-text-tertiary tracking-widest">{m.l}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="flex flex-col h-full gap-8">
              ...
              <div className="flex items-start gap-5">
                <div className="relative w-12 h-12 rounded-full border border-white/[0.12] flex items-center justify-center shrink-0 mt-1">
                  <svg className="w-5 h-5 text-text-ghost" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <h2 className="font-noto text-5xl md:text-[5.5rem] font-black tracking-tighter uppercase leading-[0.82]">
                    The Trust<br />
                    <span className="text-text-dim">Dimension.</span>
                  </h2>
                </div>
              </div>
                ...
                {[
                  ...
                  <div key={item.title} className="flex flex-col gap-3 p-6 bg-brand-bg">
                    <div className="flex items-center gap-3">
                      ...
                      <h4 className="font-noto font-black uppercase text-sm text-text-secondary">{item.title}</h4>
                    </div>
                    <p className="font-mono text-fine text-text-tertiary leading-relaxed pl-7">{item.body}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <ScrollReveal direction="right">
            <div className="flex flex-col h-full gap-8">
              ...
              <div className="flex flex-col gap-3">
                ...
                <h2 className="font-noto text-5xl md:text-[5.5rem] font-black tracking-tighter uppercase leading-[0.82]">
                  The<br />
                  <span className="text-text-ghost">Stack.</span>
                </h2>
              </div>
              ...
              <div className="grid grid-cols-2 gap-0 border border-white/[0.07] divide-x divide-y divide-white/[0.07]">
                {STACK.map((item, i) => {
                  ...
                  const content = (
                    <>
                      <span className="font-mono text-micro uppercase font-black text-text-tertiary tracking-widest">{item.l}</span>
                      <span className={cn(
                        "font-noto text-xl md:text-2xl font-black group-hover:translate-x-0.5 transition-transform duration-200",
                        isLink && "text-brand-primary"
                      )}>{item.v}</span>
                    </>
                  );
                  return (
                    ...
                    >
                      {isLink ? (
                        <a
                          ...
                          className="w-full h-full flex flex-col items-start gap-1.5"
                        >
                          {content}
                        </a>
                      ) : (
              ...
              <div className="mt-auto flex items-center gap-4 p-5 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
                ...
                <p className="font-mono text-micro text-text-tertiary leading-relaxed">
