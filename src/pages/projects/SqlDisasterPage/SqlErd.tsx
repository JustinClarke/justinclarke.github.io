    ...
    <div className="w-full relative overflow-hidden rounded-[24px] md:rounded-[40px] border border-white/10 bg-[#0a0c10] shadow-2xl">
      ...
      <div className="flex items-center justify-between px-6 md:px-10 py-4 md:py-6 border-b border-white/5 bg-white/[0.01]">
        <div className="flex items-center gap-4">
          ...
          <span className="font-mono text-micro md:text-micro tracking-[0.4em] uppercase text-text-tertiary font-black">
            RELATIONAL_MAP
          </span>
        </div>
        <div className="hidden md:flex items-center gap-4 font-mono text-micro md:text-micro text-text-tertiary uppercase tracking-widest font-black">
            ...
            return (
              <g key={i}>
                ...
                <text 
                  ...
                  y={(start.y + end.y) / 2 + 3} 
                  textAnchor="middle" 
                  className="font-mono text-micro fill-white/60 font-black"
                >
                  {conn.label}
                </text>
              </g>
          ...
          {ENTITIES.map((entity, idx) => (
            ...
            >
              <div className="px-4 py-2.5 border-b border-white/5 bg-white/[0.02] flex items-center justify-between">
                <span className="font-noto text-micro font-black uppercase tracking-tight text-white">
                  {entity.name}
                </span>
                ...
                {entity.fields.map((field) => (
                  <div key={field.name} className="px-3 py-1.5 flex items-center justify-between rounded-md hover:bg-white/[0.03] transition-colors group">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-micro text-text-secondary group-hover:text-white">{field.name}</span>
                    </div>
                    <span className="font-mono text-micro text-text-tertiary uppercase font-black">{field.type}</span>
                  </div>
                ))}
              </div>
            </motion.div>
      ...
      <div className="flex md:hidden flex-col items-center justify-center p-12 text-center gap-4 bg-white/[0.02]">
         <div className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center">
            <span className="font-mono text-lg text-text-ghost">!</span>
         </div>
         <span className="font-mono text-micro text-text-tertiary uppercase font-black tracking-widest">Optimized for Desktop Interface</span>
      </div>
      ...
      <div className="px-6 md:px-10 py-4 border-t border-white/5 bg-[#080a0e] flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex gap-4">
           <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-micro text-text-tertiary uppercase font-bold">PK</span>
           </div>
           <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-sm bg-purple-500/50" />
              <span className="font-mono text-micro text-text-tertiary uppercase font-bold">FK</span>
           </div>
        </div>
        <div className="font-mono text-micro text-text-tertiary uppercase tracking-widest">
