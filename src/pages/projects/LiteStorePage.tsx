...
export const LiteStorePage = () => {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-surface text-fg font-sans selection:bg-litestore/30 overflow-x-hidden">
      <SEO {...routeMeta('/project/litestore')} />
      ...
      <div className="fixed top-12 right-6 md:right-12 z-[100] flex items-center gap-3">
        <div className="hidden md:flex px-4 py-1.5 rounded-full border border-white/10 bg-black/60 backdrop-blur-md items-center gap-3">
          ...
          <span className="font-mono text-micro tracking-[0.4em] uppercase text-text-tertiary">Production Live</span>
        </div>
        <a
          href="https://litestore.in/"
          target="_blank"
          rel="noopener noreferrer"
          className="px-5 py-2 rounded-full bg-litestore text-white font-mono text-micro font-black uppercase tracking-widest shadow-xl hover:scale-105 transition-all duration-200"
        >
          Visit Site
        </a>
      </div>
        ...
        <ScrollReveal direction="up">
          <div className="relative z-10 max-w-6xl mx-auto w-full">
            <div className="inline-flex items-center gap-3 mb-8 px-5 py-2 rounded-full border border-white/8 bg-white/[0.03]">
              ...
              <span className="font-mono text-micro md:text-micro tracking-[0.5em] uppercase font-bold text-text-tertiary">
            ...
            <p className="font-mono text-base md:text-xl text-text-tertiary max-w-2xl leading-relaxed mb-10">
        ...
        <div className="relative z-10 max-w-6xl mx-auto w-full mt-16 pt-8 border-t border-white/8 grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-micro tracking-widest uppercase text-text-tertiary font-black">Build Output</span>
            </div>
            <div className="font-mono text-micro md:text-xs text-litestore/70 h-5">
          ...
          <div className="flex gap-10 md:justify-end">
            {[
              ...
              <div key={m.l} className="flex flex-col gap-0.5 text-right">
                <span className="font-mono text-micro text-text-tertiary uppercase tracking-widest">{m.l}</span>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <ScrollReveal direction="left">
            <div className="flex flex-col gap-8">
              <div>
                <span className="font-mono text-micro tracking-[0.5em] uppercase text-litestore font-black">Architecture</span>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {STACK_PILLS.map(pill => (
                  <span
                    key={pill}
                    className="px-3 py-1 rounded-full border border-white/10 bg-white/[0.03] font-mono text-micro tracking-wider text-text-tertiary uppercase"
                  >
                    {pill}
                  </span>
                ))}
              ...
              <div className="p-6 rounded-2xl border border-white/5 bg-white/[0.02] flex flex-col gap-4">
                <span className="font-mono text-micro uppercase tracking-widest text-text-tertiary font-black">Form Pipeline</span>
                <div className="flex items-center gap-3 flex-wrap">
                  {[
                    { label: 'Next.js Form', sub: 'client-side', color: 'border-litestore/30 text-litestore' },
                    { label: '→', sub: '', color: 'text-text-dim border-transparent' },
                    ...
                    step.label === '→'
                      ? <span key={i} className="font-mono text-text-dim text-lg">→</span>
                      : (
                        <div key={i} className={`px-3 py-2 rounded-xl border ${step.color} flex flex-col`}>
                          <span className="font-mono text-micro font-black uppercase tracking-wider">{step.label}</span>
                          {step.sub && <span className="font-mono text-micro text-text-tertiary">{step.sub}</span>}
                        </div>
                      )
                  ))}
                </div>
          ...
          <ScrollReveal direction="right">
            <div className="flex flex-col gap-8">
              <div>
                <span className="font-mono text-micro tracking-[0.5em] uppercase text-litestore font-black">Design System</span>
              ...
              <div className="relative p-1 bg-gradient-to-br from-litestore/20 to-transparent rounded-[28px]">
                <div className="bg-[rgb(13,13,13)] rounded-[27px] p-6 border border-white/5">
                  <div className="flex items-center gap-2 mb-5 opacity-40">
                    ...
                    <span className="ml-3 font-mono text-micro uppercase tracking-widest text-text-tertiary">tailwind.config.js · theme.colors</span>
                  </div>
                  <div className="flex flex-col gap-2">
                    {BRAND_TOKENS.map(token => (
                      <div key={token.name} className="flex items-center gap-4 group">
                        ...
                        <div className="flex items-baseline gap-3 min-w-0">
                          ...
                          <code className="font-mono text-micro text-text-tertiary">{token.hex}</code>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <section className="py-20 md:py-28 px-6 md:px-16 bg-brand-bg text-white border-b border-white/5">
        <div className="max-w-6xl mx-auto">
          <ScrollReveal direction="up">
            <div className="mb-12">
              <span className="font-mono text-micro tracking-[0.5em] uppercase text-litestore font-black">Production Metrics</span>
              ...
              <ScrollReveal key={m.label} delay={i * 0.06} direction="up" className="h-full">
                <div className="group relative p-6 h-full rounded-2xl border border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent flex flex-col gap-4 hover:border-litestore/30 hover:bg-white/[0.03] hover:-translate-y-1 transition-all duration-300">
                  <div className="flex justify-between items-center">
                    <span className="font-mono text-micro text-text-muted font-bold uppercase tracking-widest">[METRIC_0{i + 1}]</span>
                  ...
                  <div className="flex flex-col gap-1.5 mt-auto">
                    ...
                    <span className="font-mono text-micro text-text-tertiary leading-relaxed uppercase tracking-wider group-hover:text-text-secondary transition-colors duration-300">{m.label}</span>
                  </div>
        ...
        <div className="max-w-6xl mx-auto">
          <ScrollReveal direction="up">
            <div className="mb-12 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                <span className="font-mono text-micro tracking-[0.5em] uppercase text-litestore font-black">What I Actually Did</span>
            ...
            {CONTRIBUTIONS.map((c, i) => (
              <ScrollReveal key={c.label} delay={i * 0.07} direction="up">
                ...
                >
                  ...
                  <p className={`font-mono text-sm leading-relaxed ${c.accent ? 'text-text-secondary' : 'text-black/50'}`}>
                    {c.detail}
                  </p>
                </div>
              </ScrollReveal>
